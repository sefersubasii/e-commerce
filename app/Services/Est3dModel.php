...
class est3dModel
{
    public function __construct($test = false)
    {
        ...
        if ($test) {
            $this->client_id = "600100000";
            $this->storekey  = "123456BDR";
            $this->name      = "BDRTEST";
            $this->password  = "BDRTEST1021**";
            $this->live_url  = "https://entegrasyon.asseco-see.com.tr/fim/est3Dgate";
            $this->api_url   = "https://entegrasyon.asseco-see.com.tr/fim/api";

            // $this->client_id = "700655000100";
            // $this->storekey = "TRPS1234";
            // $this->name = "ISBANKAPI";
            ...
            $this->api_url   = "https://sanalpos.isbank.com.tr/fim/api";
            $this->client_id = "700659815021";
            $this->storekey  = "123aweZXC";
            $this->name      = "myapi";
            $this->password  = "*mK17eR@";
    ...
    public function bankResponse($bank_response, $bank = null)
    {
        ...
        while ($index1 < strlen($hashparams)) {
            ...
            $vl     = isset($bank_response[substr($hashparams, $index1, $index2 - $index1)]) ? $bank_response[substr($hashparams, $index1, $index2 - $index1)] : null;
            if ($vl == null) {
                $vl = "";
            }
        ...
        $mdArray  = array('1', '2', '3', '4', '5', '7', '8');
        $sonuc    = array();

        $sonuc['message'] = "";
        if (in_array($mdStatus, $mdArray)) {
            $sonuc['message'] = '3D Onayı Başarılı.';
            ...
            if ($result["Response"] === "Approved") {
                $sonuc['result']   = 1;
                $sonuc['message']  = 'Ödeme Başarılı<br/>';
                $sonuc['response'] = $result;
            } else {
                $sonuc['result'] = 0;
                $sonuc['message'] .= 'Ödeme Başarısız.' . @$result["ErrMsg"] . ' (' . @$result["Response"] . ')';
            }
        } else {
            $sonuc['result'] = 0;
            $sonuc['message'] .= '3D doğrulama başarısız.';
            $sonuc['message'] .= 'mdStatus=' . @$mdStatus . ' - ' . @$bank_response['ErrMsg'];
        }
        return $sonuc;
    }
    ...
    private function xmlSend($fields)
    {
        ...
        // curl_setopt($ch, CURLOPT_URL, ; // set url to post to
