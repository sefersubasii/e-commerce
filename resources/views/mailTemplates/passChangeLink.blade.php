


<table width="600" border="0" cellspacing="5" style="color: #666; font-size: 14px; font-family: arial; border: solid 2px #ed1d24;text-align: center; font-weight: 100; ">
  <tbody>
    <tr>
      <th><img src="https://marketpaketi.com.tr/images/market-paketi-gorsel.jpg" width="700" height="150" alt=""/></th></tr>
    <tr>
     <tr><th><strong style="color: #ed1d24; font-size: 18px">Marketpaketi.com.tr</strong></th></tr>
     
      <th scope="col">
        
        <table width="700" height="30" align="center" cellpadding="0" cellspacing="0" border="0" style="text-align: left; font-weight: 100; line-height: 22px">
          <tbody>
            <tr>
              <td width="15%"><strong style="text-align: center; width: 98%; height: 40px; line-height: 40px; background: #262262; display: block;color: #fff; padding-left: 2%; margin: 10px 0px">Şifre Yenileme Talebi</strong></td>
            </tr>
          </tbody>
        </table>
        <table width="700" height="30" align="center" cellpadding="0" cellspacing="0" border="0" style="text-align: left; font-weight: 100; line-height: 22px">
          <tbody>
            <tr>
              <td width="15%"><p><br>

              		<p>Merhaba {{$name}},</p> 
                
                	<p><a href="{{url('./')}}">{{url('./')}}</a> şifrenizi değiştirmek için bir talepte bulundunuz. Şifrenizi değiştirmek için lütfen <a href="{{url('sifremi-unuttum/'.$token)}}">tıklayınız</a></p>

                	<p>Linke tıklayamıyorsanız aşağıdaki linki kopyalayıp adres çubuğuna yapıştırarak yeni şifrenizi alabilirsiniz. 
					{{url('sifremi-unuttum/'.$token)}}</p>

					Şifre değiştirme talebi sizin tarafınızdan oluşturulmadıysa bu maili dikkate almayınız. 
            	</td>
            </tr>
          </tbody>
        </table>
        <table width="700" height="30" align="center" cellpadding="0" cellspacing="0" border="0" style="text-align: center; font-weight: 100; line-height: 22px">
          <tbody>
            <tr>
              <td> </td>
            </tr>
          </tbody>
        </table>
        <table width="40%" height="30" align="center" cellpadding="0" cellspacing="0" border="0" style="text-align: center; font-weight: 100; line-height: 22px">
          <tbody>
            <tr>
              <td><strong>Öz Koru Sanal Mağazacılık Gıda İnş. Paz. San. Dış Tic Ltd. Şti</strong><br>
                Plevne Caddesi No:272/2 Mamak Ankara<br>
                <strong>Telefon 1 :</strong> <a href="tel:+90%20312%20364%2082%2000" value="+903123648200" target="_blank">+90 312 364 82 00</a>- <strong>Telefon 2 :</strong> <br>
                <a href="http://www.marketpaketi.com.tr/" target="_blank" data-saferedirecturl="https://www.google.com/url?hl=tr&q=http://www.marketpaketi.com.tr&source=gmail&ust=1506601653359000&usg=AFQjCNHppN7-C4mFFa_Nf304fgzo31-3jQ">www.marketpaketi.com.tr</a></td>
            </tr>
          </tbody>
      </table></th>
    </tr>
  </tbody>
</table>