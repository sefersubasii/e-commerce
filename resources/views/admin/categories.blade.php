...
/*
        var editor; // use a global for the submit and return data rendering in the examples
         ...
         editor = new $.fn.dataTable.Editor( {
            ajax: "{{url('admin/categories/shortUpdateSort')}}",
            table: "#categories",
            fields: [ {
                    label: "sort",
                    name: "sort"
        ...
        $('#categories').on( 'click', 'tbody td.editable', function (e) {
            editor.inline( this );
        } );
*/
        $(function() {
            $('#categories').DataTable({
                ...
                columns: [
                    ...
                    {data: 'sort', name: 'sort', orderable: false, searchable: false, className: 'editable'},
                    ...
                    { "mRender": function ( data, type, row ) {
                        return '<a href="{{url('./')}}/admin/categories/edit/'+row.id+'" class="btn btn-xs btn-success btn-rounded"><i class="glyphicon glyphicon-edit"></i> Düzenle</a>';                    
                    },orderable: false, className:'text-center'},
                    //{data: 'delete', name: 'delete', orderable: false, searchable: false}
                    { "mRender": function ( data, type, row ) {
                        return '<a href="{{url('./')}}/admin/categories/delete/'+row.id+'" onclick="return confirm(\'Silmek İstediğinize Eminmisiniz?\')" class="btn btn-xs btn-danger btn-rounded"><i class="glyphicon glyphicon-remove"></i> Sil</a>';                    
                    },orderable: false, className:'text-center'}
                ],
                "columnDefs": [
                    { "width": "20", "targets": 5 },
